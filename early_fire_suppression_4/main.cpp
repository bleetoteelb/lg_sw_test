...
int testInput_2_1[5] = { 3, 7, 4, 2, 1 }; //TODO: 문제 (2-1) 1, 1, 1, 1, 1 => 1번 드론만 출동하는 경우
int testInput_2_2[5] = { 6, 5, 1, 2, 4 }; //TODO: 문제 (2-2) 2, 2, 2, 2, 2 => 2번 드론만 출동하는 경우
int testInput_2_3[5] = { 3, 7, 6, 5, 4 }; //TODO: 문제 (2-3) 1, 1, 2, 2, 1 => 1, 2번 병행 출동하는 경우
int testInput_2_4[5] = { 3, 6, 5, 7, 1 }; //TODO: 문제 (2-4) 1, 2, 2, 1, 2 => 1, 2번 병행 출동하는 경우


// 문제 3. 테스트 코드 작성
class FireSensor_test : public FireSensor
{
private:
    int cnt;
    int* order;
...
public:
    FireSensor_test(int *k):cnt(0), order(k){}
    POS getSensorPosition(){
        return sensorTable[order[cnt++]];
    }
};
...
void test_3_1()
{
    ...
    int order[5] = {1,7,2,3,5};
    FireSensor_test fst(order);
    DroneController dc(&fst);

    int cnt = 5;
    while(cnt--) dc.selectDrone();
}
